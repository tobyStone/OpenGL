...
float yaw = -90.0f; //yaw initialised to this because 0.0f is along the positive x-axis
float pitch = 0.0f;
float fov = 45.0f;
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	float yoffset = lastY - ypos; // reversed as y coordinates run from bottom to top
	lastX = xpos;
	lastY = ypos;

	float sensitivity = 0.3f;// to taste
	xoffset *= sensitivity;
	yoffset *= sensitivity;
